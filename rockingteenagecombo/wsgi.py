...
def create_wsgi_request(event_info,
    ...
    query = event_info[
        'queryStringParameters']  # APIGW won't allow multiple entries,
    # ex ?id=a&id=b
    headers = event_info[
                  'headers'] or {}  # Allow for the AGW console 'Test' button
    # to work (Pull #735)
    ...
    if ',' in x_forwarded_for:
        # The last one is the cloudfront proxy ip. The second to last is the
        # real client ip.
    ...
    if event_info['requestContext'].get('authorizer'):
        environ['API_GATEWAY_AUTHORIZER'] = event_info['requestContext'][
            'authorizer']
...
def common_log(environ, response, response_time=None):
    ...
    if response_time:
        ...
        try:
            ...
            # Upstream introduced a very annoying breaking change on the
            # rt_ms/rt_us kwarg.
            log_entry = formatter(response.status_code, environ,
