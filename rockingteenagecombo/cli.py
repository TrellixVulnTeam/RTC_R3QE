...
class ZappaCLI(object):
            ...
            if stage in extended_stages:
                raise RuntimeError(
                    stage + " has already been extended to these settings. "
                            "There is a circular extends within the settings "
                            "file."
                )
    ...
    def override_stage_config_setting(self, key, val):
        """
        Forcefully override a setting set by zappa_settings (for the current
        stage only)
    ...
    def handle(self, argv=None):
        ...
        desc = (
            "Zappa - Deploy Python applications to AWS Lambda" " and API "
            "Gateway.\n"
        )
        ...
        all_help = "Execute this command for all of our defined " "Zappa " \
                   "stages."
        ...
        cert_parser.add_argument(
            ...
            action="store_true",
            help=(
                "Gets new Let's Encrypt certificates, but prints them to "
                "console."
        ...
        tail_parser.add_argument(
            "--force-color",
            action="store_true",
            help="Force coloring log tail output even if coloring support is "
                 "not auto-detected. (example: piping)",
        )
        tail_parser.add_argument(
            "--disable-keep-open",
            action="store_true",
            help="Exit after printing the last available log, rather than "
                 "keeping the log open.",
        )
        ...
        update_parser.add_argument(
            "-n",
            "--no-upload",
            help="Update configuration where appropriate, but don't upload "
                 "new code",
        )
        ...
        if args.color == "never":
            ...
            # TODO: Support aggressive coloring like "--force-color" on all
            #  commands
            pass
        ...
        if command == "template":  # pragma: no cover
            ...
            if len(command_tail) > 1:
                ...
                command = command_tail[
                    0]  # ex: zappa manage dev showmigrations admin
            ...
            self.certify(no_confirm=self.vargs["yes"],
    ...
    def template(self, lambda_arn, role_arn, output=None, json=False):
        ...
        with open(template_file, "wb") as out:
            out.write(
                bytes(template.to_json(indent=None, separators=(",", ":")),
                      "utf-8")
            )
    ...
    def deploy(self, source_zip=None):
        ...
        if not source_zip:
            ...
            deployed_versions = self.zappa.get_lambda_function_versions(
                self.lambda_name
            )
            ...
            if self.manage_roles:
                try:
                    ...
                    raise ClickException(
                        ...
                        + "!\n"
                        + "You may "
                        + click.style("lack the necessary AWS permissions",
                                      bold=True)
                        + " to automatically manage a Zappa execution role.\n"
                        + "To fix this, see here: "
                        + click.style(
                            "https://github.com/Miserlou/Zappa#custom-aws-iam"
            ...
            # If using a slim handler, upload it to S3 and tell lambda to use
            ...
            if self.stage_config.get("slim_handler", False):
                ...
                current_project_name = ("{0!s}_{1!s}_current_"
                ...
                if not success:  # pragma: no cover
                    raise ClickException(
                        "Unable to copy the zip to be the current project. "
                        "Quitting."
                    )
        ...
        if self.use_apigateway:
            ...
            if self.api_key_required:
                if self.api_key is None:
                    self.zappa.create_api_key(api_id=api_id,
                                              stage_name=self.api_stage)
                else:
                    self.zappa.add_api_stage_to_api_key(
                        api_key=self.api_key, api_id=api_id,
                        stage_name=self.api_stage
                    )
    ...
    def update(self, source_zip=None, no_upload=False):
        ...
        if not source_zip:
            ...
            try:
                ...
                function_response = self.zappa.lambda_.get_function(
            ...
            if last_updated_unix <= updated_time:
                click.echo(
                    click.style("Warning!", fg="red")
                    + " You may have upgraded Zappa since deploying this "
            ...
            if not no_upload:
                ...
                if not success:  # pragma: no cover
                    raise ClickException("Unable to upload project to S3. "
                                         "Quitting.")

                # If using a slim handler, upload it to S3 and tell lambda to
                ...
                if self.stage_config.get("slim_handler", False):
                    ...
                    # Copy the project zip to the current project zip
                    current_project_name = (
                        "{0!s}_{1!s}_current_project.tar.gz".format(
                            self.api_stage, self.project_name
                        )
                    )
                    success = self.zappa.copy_on_s3(
                    ...
                    if not success:  # pragma: no cover
                        raise ClickException(
                            "Unable to copy the zip to be the current "
                            "project. Quitting."
                        )
        ...
        if self.use_apigateway:
            ...
            # It looks a bit like we might actually be using this just to get
            # the URL,
        ...
        # do this after schedule as schedule clears the lambda policy and we
        # need to add one
        ...
        if self.use_apigateway:
            ...
            if endpoint_url and "amazonaws.com" not in endpoint_url:
                api_url = self.zappa.get_api_url(self.lambda_name,
                                                 self.api_stage)
            ...
            events.append(
                {
                    ...
                    "description": "Zappa Keep Warm - {}".format(
                        self.lambda_name),
                }
            )
        ...
        if events:
            try:
                ...
                click.echo(
                    ...
                    + "first. Ex:"
                    + click.style("zappa deploy {}.".format(self.api_stage),
                                  bold=True)
                )
                sys.exit(-1)
        ...
        if table_name and self.stage_config.get("async_resources", True):
            ...
            if created:
                ...
                if (
                    ...
                    click.echo(
                        click.style(
                            "\nWarning! Existing DynamoDB table ({}) does not "
                            "match configured capacity.\n".format(table_name),
                            fg="red",
                        )
                    )
    ...
    def unschedule(self):
        ...
        # Run even if events are not defined to remove previously existing
        # ones (thus default to []).
        events = self.stage_config.get("events", [])
    ...
    def invoke(self, function_name, raw_python=False, command=None,
        ...
        response = self.zappa.invoke_lambda_function(
            self.lambda_name, json.dumps(command),
    ...
    def status(self, return_json=False):
        ...
        def tabular_print(title, value):
            ...
            click.echo(
                "%-*s%s" % (
                    32, click.style("\t" + title, fg="green") + ":", str(value))
            )
            return

        # Lambda Env Details
        lambda_versions = self.zappa.get_lambda_function_versions(
            self.lambda_name)
        ...
        try:
            error_rate = "{0:.2f}%".format(
                function_errors / function_invocations * 100)
        except:
        ...
        if self.use_apigateway:
            ...
            # AWS provides no way to tie a APIGW domain name to its Lambda
            # function.
        ...
        # Scheduled Events
        event_rules = self.zappa.get_event_rules_for_lambda(
            lambda_arn=self.lambda_arn)
        ...
        for rule in event_rules:
            ...
            event_dict["Event Rule Schedule"] = rule.get("ScheduleExpression",
                                                         None)
        ...
        if return_json:
            ...
            click.echo(
                "Status for " + click.style(self.lambda_name, bold=True) + ": ")
    ...
    def check_environment(self, environment):
        ...
        if non_strings:
            raise ValueError(
                f"The following environment variables are not strings: "
                f"{', '.join(non_strings)}"
            )
        else:
            return True
    ...
    def init(self, settings_file="zappa_settings.json"):
        """
        Initialize a new Zappa project by creating a new zappa_settings.json
        in a guided process.

        This should probably be broken up into few separate componants once
        it's stable.
        Testing these inputs requires monkeypatching with mock, which isn't
        pretty.
        ...
        if os.path.isfile(settings_file):
            raise ClickException(
                ...
                + click.style("{0!s} file".format(settings_file), fg="red",
                              bold=True)
                + "!"
            )
...
╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝     ╚═╝  ╚═╝\n""",
        ...
        click.echo(
            "This `init` command will help you create and configure your new "
            "Zappa deployment."
        )
        ...
        while True:
            ...
            env = (
                    input(
                        "What do you want to call this environment (default "
                        "'dev'): ")
                    or "dev"
            )
            try:
        ...
        click.echo(
            "\nAWS Lambda and API Gateway are only available in certain "
            "regions. "
            "Let's check to make sure you have a profile set up in one that "
            "will work."
        )
        ...
        if not profile_names:
            ...
            click.echo(
                ...
                "you'll need to set one up. See here for more info: {}".format(
                    click.style(BOTO3_CONFIG_DOCS_URL, fg="blue",
                                underline=True)
                )
            )
        ...
        while True:
            ...
            click.echo(
                """* Bucket names must be unique across all existing bucket 
                names in Amazon S3.
...
separated 
  by a single period (.). Bucket names can contain lowercase letters, 
  numbers, and
...
the SSL 
  wildcard certificate only matches buckets that don't contain periods. To 
  work around 
  this, use HTTP or write your own certificate verification logic. We 
  recommend that 
  you do not use periods (".") in bucket names when using virtual 
...
"""
        ...
        while app_function in [None, ""]:
            if matches:
                ...
                app_function = (
                        input("Where is your app's function? (default '%s'): " %
                              matches[0])
                        or matches[0]
                )
            else:
        ...
        click.echo(
            "If you are using Zappa for the first time, you probably don't "
            "want to do this!"
        )
        ...
        if global_deployment:
            additional_regions = [r for r in API_GATEWAY_REGIONS if
                                  r != profile_region]
            # Create additional stages
            if global_type.lower() in ["p", "primary"]:
                additional_regions = [r for r in additional_regions if
                                      "-1" in r]
        ...
        click.echo(
            ...
            + " here: "
            + click.style("https://github.com/Miserlou/Zappa", fg="cyan",
                          bold=True)
        )
        click.echo(
            "and stop by our "
    ...
    def certify(self, no_confirm=True, manual=False):
        ...
        if not cert_location and not cert_arn:
            ...
            cert_success = get_cert_and_update_domain(
                self.zappa, self.lambda_name, self.api_stage, self.domain,
                manual
            )
        ...
        # Custom SSL / ACM
            ...
            if not self.zappa.get_domain_name(self.domain, route53=route53):
                ...
                print(
                    ...
                    "Please note that it can take up to 40 minutes for this "
                    "domain to be "
                    "created and propagated through AWS, but it requires no "
        ...
        if cert_success:
            click.echo(
                "Certificate " + click.style("updated", fg="green",
                                             bold=True) + "!"
            )
        else:
            click.echo(
    ...
    def callback(self, position):
        """
        Allows the execution of custom code between creation of the zip file
        ...
        if callback:
            ...
            try:  # Prefer callback in working directory
                ...
                try:  # Callback func might be in virtualenv
                    ...
                    raise ClickException(
                        ...
                        + "to "
                        + click.style(
                            "import {position} callback ".format(
                                position=position),
                            bold=True,
                        )
            ...
            if not hasattr(module_, cb_func_name):  # pragma: no cover
                raise ClickException(
                    ...
                    + "to "
                    + click.style(
                        "find {position} callback ".format(position=position),
                        bold=True
                    )
    ...
    def check_for_update(self):
        ...
        try:
            ...
            if updateable:
                ...
                click.echo(
                    "Visit the project page on GitHub to see the latest "
                    "changes: "
    ...
    def load_settings(self, settings_file=None, session=None):
        ...
        An existing boto session can be supplied, though this is likely for
        testing purposes.
        ...
        for stage_name in self.zappa_settings.keys():
            try:
                ...
                raise ValueError(
                    "API stage names must match a-zA-Z0-9_ ; '{0!s}' does "
                    "not.".format(stage_name)
                )
        ...
        # We need a working title for this project. Use one if supplied,
        # else cwd dirname.
        if "project_name" in self.stage_config:  # pragma: no cover
            # If the name is invalid, this will throw an exception with
            # message up stack
        ...
        if (
            ...
            raise ClickException(
                "Please supply either an integer or null for "
                "num_retained_versions in the zappa_settings.json. Found %s"
                % type(self.num_retained_versions)
            )
        elif type(
                self.num_retained_versions) is int and \
                self.num_retained_versions < 1:
            raise ClickException(
                "The value for num_retained_versions in the "
                "zappa_settings.json should be greater than 0."
            )
        ...
        # Additional tags
        self.tags = self.stage_config.get("tags", dict())
        ...
        self.config = dict(
            ...
            runtime=self.runtime,
            tags=self.tags,
            endpoint_urls=self.stage_config.get("aws_endpoint_urls", dict()),
            xray_tracing=self.xray_tracing,
        )
        self.zappa = Zappa(**self.config)
        ...
        if self.app_function:
            ...
            if self.app_function[-3:] == ".py":
                click.echo(
                    ...
                    + "! It should probably be something like 'my_file.app', "
                      "not 'my_file.py'!"
                )
    ...
    def load_settings_file(self, settings_file=None):
        ...
        if not os.path.isfile(settings_file):
            raise ClickException(
                "Please configure your zappa_settings file or call `zappa "
                "init`."
            )
        ...
        if ext == ".yml" or ext == ".yaml":
            ...
            with open(settings_file) as toml_file:
                try:
                    ...
                    raise ValueError(
                        "Unable to load the Zappa settings TOML. It may be "
                        "malformed."
                    )
        else:
            with open(settings_file) as json_file:
                try:
                    ...
                    raise ValueError(
                        "Unable to load the Zappa settings JSON. It may be "
                        "malformed."
                    )
    ...
    def create_package(self, output=None):
        ...
        # Create the Lambda zip package (includes project and
        # virtualenvironment)
        ...
        handler_file = (
                os.sep.join(
                    current_file.split(os.sep)[0:]) + os.sep + "handler.py"
        )
        ...
        if self.stage_config.get("slim_handler", False):
            ...
            if sys.version_info[0] < 3:
                # Exclude packages already builtin to the python lambda
                # environment
                ...
                exclude = self.stage_config.get(
                    "exclude",
                    ["boto3", "dateutil", "botocore", "s3transfer",
                     "concurrent"],
                )
            ...
            if file_stats.st_size > 52428800:  # pragma: no cover
                print(
                    "\n\nWarning: Application zip package is likely to be too "
                    "large for AWS Lambda. "
                    'Try setting "slim_handler" to true in your Zappa '
                    "settings file.\n\n"
                )
        ...
        with zipfile.ZipFile(handler_zip, "a") as lambda_zip:
            ...
            if self.stage_config.get("slim_handler", False):
                settings_s += (
                    "ARCHIVE_PATH='s3://{0!s}/{1!s}_"
                    "{2!s}_current_project.tar.gz'\n".format(
                        self.s3_bucket_name, self.api_stage, self.project_name
                    )
                )

                # since includes are for slim handler add the setting here by
                # joining arbitrary list from zappa_settings file
    ...
    def on_exit(self):
        ...
        Always called: SystemExit, KeyboardInterrupt and any other Exception
        that occurs.
        """
        if self.zip_path:
            # Only try to remove uploaded zip if we're running a command that
            # has loaded credentials
    ...
    def print_logs(
            self, logs, colorize=True, http=False, non_http=False,
            force_colorize=None
    ):
        """
    ...
    def colorize_log_entry(self, string):
        ...
        try:
            ...
            for token in inside_squares:
                if token in ["CRITICAL", "ERROR", "WARNING", "DEBUG", "INFO",
                             "NOTSET"]:
                    final_string = final_string.replace(
                        "[" + token + "]",
                        click.style("[", fg="cyan")
    ...
    def deploy_api_gateway(self, api_id):
        ...
        cache_cluster_size = str(
            self.stage_config.get("cache_cluster_size", 0.5))
        endpoint_url = self.zappa.deploy_api_gateway(
            api_id=api_id,
            stage_name=self.api_stage,
            cache_cluster_enabled=cache_cluster_enabled,
            cache_cluster_size=cache_cluster_size,
            cloudwatch_log_level=self.stage_config.get("cloudwatch_log_level",
                                                       "OFF"),
            cloudwatch_data_trace=self.stage_config.get("cloudwatch_data_trace",
                                                        False),
...
def disable_click_colors():
    """
    Set a Click context where colors are disabled. Creates a throwaway
    BaseCommand
    to play nicely with the Context constructor.
    The intended side-effect here is that click.echo() checks this context
    and will
    suppress colors.
...
def handle():  # pragma: no cover
    ...
    try:
        ...
        click.echo(
            "Oh no! An " + click.style("error occurred", fg="red",
                                       bold=True) + "! :("
        )
        click.echo("\n==============\n")
        import traceback
